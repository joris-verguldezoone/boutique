...
class Panier extends Controller
{
    ...
    { // on display aussi la somme totale a payer
        $model = new \Model\Panier();
    ...
    public function sumPrice($id_utilisateur)
    { // on fait une fonction a part pour la sécurité et double vérification du montant a payer 
        $model = new \Model\Panier();
        $tab = $model->selectAllWhereFetchAll('panier', 'id_utilisateur', $id_utilisateur);
        $sumPrix = 0;
        foreach ($tab as $value) {
